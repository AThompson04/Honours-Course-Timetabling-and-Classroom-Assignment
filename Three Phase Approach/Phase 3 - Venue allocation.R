...
for(i in 1:nrow(v_capacity)){
  for(j in 1:nrow(m_size1)){
    if(m_size1[j,1]<=v_capacity[i]){
      fitS1[i,j] <- 1 # Venue can accommodate class
    }else{
  ...
  for(k in 1:nrow(m_size2)){
    if(m_size2[k,1]<=v_capacity[i]){
      fitS2[i,k] <- 1 # Venue can accommodate class
    }else{
...
#### Semester 1 Morning Preference ####
## Preparing data
s1_morning <- read_excel("Semester1_mornings.xlsx")
...
# Venues allocation for modules that have more than one class a week
for(i in 1:s1){
...
#### Semester 1 Afternoon Preference ####
## Preparing data
s1_afternoon <- read_excel("Semester1_afternoons.xlsx")
s1after <- as.data.frame(s1_afternoon)
rownames(s1after) <- s1after[,1]
s1after <- s1after[,-1]
s1_afternoon <- as.matrix(s1after)

s1 <- nrow(contact_hours1)

# Repeating preferences per venue
c_afternoon1 <- t(matrix(as.vector(t(s1_afternoon))))
pref_available <- matrix(0, nrow = 1, ncol = ncol(c_afternoon1), ) # Multiplying the preference times by when lecturers for each module are available
for(i in 1:v){
  rough <- c_afternoon1*rep(venues1[i,],s1)
  pref_available <- rbind(pref_available,rough)
}
afternoon_S1 <- pref_available[-1,]

for(i in 1:v){
  afternoon_S1[i,] <- afternoon_S1[i,]*rep(fitS1[i,], each = 50)
}

## Running the model
system.time(as1_assig <- venue_assignemnt(afternoon_S1, contact_hours1, s1))
as1_assig
as1_assign <- matrix(as1_assig$solution[1:as.numeric(dim(afternoon_S1)[1]*dim(afternoon_S1)[2])] , dim(afternoon_S1)[1], dim(afternoon_S1)[2], byrow = TRUE )
as1_assign

answer_s1a <- matrix(0, nrow = v, ncol = s1)
row.names(answer_s1a) <- venues
modules_s1 <- rownames(contact_hours1)
colnames(answer_s1a) <- modules_s1
# Venue allocation for modules with more than one class a week
for(i in 1:s1){
  answer_s1a[,i] <- t(t(rowSums(as1_assign[,((i-1)*50+1):(i*50)])))
  if(contact_hours1$`Classes a Week`[i]==2){ # If more than one class a week print the solution so we can see when each venue was booked
    x <- as1_assign[,((i-1)*50+1):(i*50)]
    for(j in 1:nrow(x)){
      for(k in 1:ncol(x)){
        if(x[j,k]>=1){
          if(k%%10!=0){
...
row.names(answer_s1a) <- ven_names
# Venue Allocation
answer_s1a


# Checking that venues were allocated at the correct time
colSums(as1_assign)==as.vector(t(s1_afternoon))

write.xlsx(answer_s1a, "Semester1_afternoon_venues.xlsx")

#### Semester 2 Morning Preference ####
## Preparing data
s2_morning <- read_excel("Semester2_mornings.xlsx")
...
#### Semester 2 Afternoon Preference ####
# Preparing data
s2_afternoon <- read_excel("Semester2_afternoons.xlsx")
s2after <- as.data.frame(s2_afternoon)
rownames(s2after) <- s2after[,1]
s2after <- s2after[,-1]
s2_afternoon <- as.matrix(s2after)

s2 <- nrow(contact_hours2)

# Repeating preferences per venue
c_afternoon2 <- t(matrix(as.vector(t(s2_afternoon))))
pref_available <- matrix(0, nrow = 1, ncol = ncol(c_afternoon2), ) # Multiplying the preference times by when lecturers for each module are available
for(i in 1:v){
  rough <- c_afternoon2*rep(venues1[i,],s2)
  pref_available <- rbind(pref_available,rough)
}
afternoon_S2 <- pref_available[-1,]

for(i in 1:v){
  afternoon_S2[i,] <- afternoon_S2[i,]*rep(fitS2[i,], each = 50)
}

## Running the model
system.time(as2_assig <- venue_assignemnt(afternoon_S2, contact_hours2, s2))
as2_assig
as2_assign <- matrix(as2_assig$solution[1:as.numeric(dim(afternoon_S2)[1]*dim(afternoon_S2)[2])] , dim(afternoon_S2)[1], dim(afternoon_S2)[2], byrow = TRUE )
as2_assign

answer_s2a <- matrix(0, nrow = v, ncol = s2)
row.names(answer_s2a) <- venues
modules_s2 <- rownames(contact_hours2)
colnames(answer_s2a) <- modules_s2
# Venue allocation for modules with more than one class a week
for(i in 1:s2){
  answer_s2a[,i] <- t(t(rowSums(as2_assign[,((i-1)*50+1):(i*50)])))
  if(contact_hours2$`Classes a Week`[i]==2){ # If more than one class a week print the solution so we can see when each venue was booked
    x <- as2_assign[,((i-1)*50+1):(i*50)]
    for(j in 1:nrow(x)){
      for(k in 1:ncol(x)){
        if(x[j,k]>=1){
          if(k%%10!=0){
...
row.names(answer_s2a) <- ven_names
# Venue allocation
answer_s2a


# Checking that venues were allocated at the correct time
colSums(as2_assign)==as.vector(t(s2_afternoon))

write.xlsx(answer_s2a, "Semester2_afternoon_venues.xlsx")
